...
- [ ] `FUT-016` `P1 P` Шрифты. База сделана 2026-09-24: в списке только
  ...
  подходящий набор или сам этот шрифт. Архивы наборов лежат в выпуске
  `fonts-2` на GitHub; его собирает и публикует
  `.github/workflows/fonts.yml` по метке `fonts-*`, и только если сборка в
  точности повторяет `src/font-pack-catalogue.ts`. Это предварительный
  выпуск: его пропускают и «последний выпуск» GitHub, который читает
  проверка обновлений, и BRAT, который прочёл бы `fonts-2` как версию
  2.0.0. Если набора в выпуске нет, скачивание прямо говорит «этот набор
  ещё не выложен» вместо ошибки.
