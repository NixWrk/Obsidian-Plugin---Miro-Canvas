...
или стрелку, скопировать, вырезать и удалить. На desktop сочетания буфера
используют системный буфер и физические C/X/V, включая русскую раскладку.
Файловые ноды сохраняют ссылку на исходный файл.
Команда `Miro Canvas: Reset tools and selection` по умолчанию назначена на
Escape; переназначение — в Settings → Hotkeys Obsidian.
...
свободными точками. Прямоугольная рамка выделения Obsidian теперь захватывает
независимые соединители при пересечении маршрута с рамкой, даже если оба конца
снаружи. Удаление проверяет блокировки всех
...
геометрией при перетаскивании по кадрам анимации. Тест проверяет это с отключённым
...
а не правка метаданных Miro. Стрелки привязываются к значкам комментариев и
следуют за ними. Размер наконечника настраивается независимо от толщины линии.
Рамка смешанного выделения охватывает и нативные элементы, и независимые
соединители; перетаскивание её границы перемещает всю группу одной транзакцией.
...
- Перестроение SVG и расчёт маршрутов требуют профилирования больших холстов
  перед обещаниями производительности.
