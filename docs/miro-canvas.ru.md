...
- [ ] `FUT-015` `P1 P` Ручная настройка слоёв: какой элемент выше, какой ниже.
  Кнопки «На передний план», «Вперёд», «Назад», «На задний план» в панели
  выделения и в меню, для одного элемента и для всего выделения с сохранением
  взаимного порядка; одна запись в истории отмены. Порядок хранится так, чтобы
  без плагина доска по возможности выглядела так же (порядок нод в файле), а
  точный порядок плагина — в `miroCanvas.zOrder`. Ядро перестановки уже есть
  (`changeZOrder`), но доступно только из панели локальных инструментов. Canvas
  рисует все рёбра под карточками, поэтому линии упорядочиваются между собой, а
  не с карточками.
