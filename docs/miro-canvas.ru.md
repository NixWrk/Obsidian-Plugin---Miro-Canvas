...
- [ ] `FUT-003` `P1 P` При первой настройке спрашивать, нужен ли импорт из Miro;
  если да — показать понятную пошаговую инструкцию с картинками и
  автоматизировать всё, что можно безопасно (открыть конвертер с уже
  подставленными хранилищем и папкой, открыть готовую доску). Кнопка запуска
  остаётся в настройках плагина. Плагин не скачивает и не запускает программы
  сам, а ведёт пользователя к установке конвертера.
