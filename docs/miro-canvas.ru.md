...
- [x] `FUT-015` `P1 P` Ручная настройка слоёв (сделано 2026-09-24). Слои есть
  ...
  обновляется вместе с ним, новый не создаётся. Перетаскивание сохраняет слой,
  как в Miro (нативный Canvas поднял бы карточку наверх). Выделенная карточка,
  как и в Canvas, рисуется поверх остальных, кроме момента сразу после смены
  слоя: тогда она видна на новом слое, пока не сменится выделение.
