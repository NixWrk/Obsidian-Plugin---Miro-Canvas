...
Canvas: ручка поворота под выделением, точка связи на середине каждой стороны и
стрелка быстрого создания. Поворот показывается на месте и записывается один раз
при отпускании, поэтому жест даёт одну запись в нативной истории, а не десятки;
...
documents**: прямоугольник, скруглённый прямоугольник, эллипс, треугольник,
ромб и звезда с редактируемым native text fallback; локальные комментарии,
ответы и resolved-статус. Импортированные комментарии доступны только для чтения.
...
неизвестные поля и записывают одну транзакцию native history.
