...
**Версионированная схема (v1, 2026-09-24).** Контракт записан как JSON Schema
(draft 2020-12) в `miro2obsidian/schemas/v1/`: `board.schema.json` (весь файл
`.canvas`: JSON Canvas 1.0 и два ключа расширения), `miro-source.schema.json` и
`miro-canvas.schema.json`. Их ведёт miro2obsidian и поставляет вместе с пакетом;
в `fixtures/` лежат маленькие правильные и неправильные доски и манифест с тем,
что о каждой должна сказать каждая проверка. Эти эталоны прогоняют и тесты
Python, и тесты плагина, а список известных плагину полей `miroCanvas` обязан
совпадать со схемой, так что стороны не разойдутся. Лишние поля разрешены везде,
где плагин сохраняет поля более новых версий. Проверить доску:
`python -m miro2obsidian.validate <file.canvas>`.
...
- [x] `FUT-001` `P1 P` Язык настроек и интерфейса — язык Obsidian, английский
  по умолчанию; есть английский и русский (сделано 2026-09-24). Все слова — в
  таблицах `src/locales/`, русская обязана иметь ровно те же ключи, тест ловит
  непереведённые строки. Диагностика для разработчиков остаётся на английском.
  Имена команд больше не повторяют «Miro Canvas:» — его добавляет сам Obsidian.
- [x] `FUT-002` `P1 P` Вынести плагин в отдельный репозиторий, сохранив связь с
  ...
  закрепляет версию схемы и прогоняет эти файлы в своём CI. Сделано 2026-09-24:
  этот репозиторий с историей плагина; `schema/v1` - копия схемы miro2obsidian,
  закреплённая `schema/pin.json`, `npm run schema:check` сверяет её в CI.
- [x] `FUT-003` `P1 P` Пользователю одного плагина код экспорта из Miro не нужен.
  Сделано 2026-09-24: при первом запуске плагин один раз спрашивает
  «Импортировать доски из Miro?»; «Показать как» открывает гид из шести шагов
  ...
  через скилл или MCP для miro2obsidian, и провести по понятной пошаговой
  инструкции с картинками. После импорта экспортёр можно удалить; кнопка повтора
  остаётся в настройках плагина. Сам плагин экспортёр не устанавливает: правила
  каталога Obsidian запрещают плагинам устанавливать себя или свои зависимости.
- [ ] `FUT-013` `P1 S` Сборки экспортёра на Python под Windows, macOS и Linux.
  Сделано 2026-09-24: `release/miro2obsidian.spec` (окно и командная строка,
...
- [x] `FUT-015` `P1 P` Ручная настройка слоёв (сделано 2026-09-24). Слои есть
  ...
  front», «Bring forward», «Send backward», «Send to back» — в меню «Слой»
  панели выделения, в нативном меню карточки и выделения и командами без
  горячих клавиш по умолчанию. Действуют на все выбранные карточки с
  сохранением их взаимного порядка, одна запись в истории отмены. «Вперёд» и
  «Назад» проводят карточку мимо ближайшей перекрывающей её карточки; если
  перекрытия нет, ничего не меняется. Порядок — это порядок нод в файле, так что
  без плагина доска выглядит так же; уже имеющийся `miroCanvas.zOrder`
  обновляется вместе с ним, новый не создаётся. Ни перетаскивание, ни выделение
  не поднимают карточку, как в Miro (нативный Canvas поднимает перетаскиваемую
  и одну выделенную карточку поверх остальных). Там, где выделенную карточку
  закрывает карточка спереди, закрыты и её рамка и ручки размера.
- [x] `FUT-014` `P2 S` Скилл для самого miro2obsidian (сделано 2026-09-24):
  `.agents/skills/miro2obsidian-import` проводит человека через приложение
  Miro, экспорт, конвертацию и частые проблемы, не касаясь его ключей.
  MCP-сервер — в фазе 6.
...
- [x] `FUT-005` `P1 S` miro2obsidian не обязывает пользоваться плагином
  ...
  поля стандарта), miro-canvas или сырой JSON.
- [x] `FUT-006` `P1 S` Дедупликация вложений по SHA-256 (сделано 2026-09-24):
  одинаковые файлы хранятся один раз в «Miro attachments», ноды ссылаются на них,
  манифест `.miro2obsidian/attachments.json` даёт следующим импортам взять то,
  что уже есть; `--keep-board-attachments` оставляет прежние папки досок.
...
- [x] `FUT-008` `P1 P` Экспорт в PDF/PPTX (сделано 2026-09-24). В меню доски —
  ...
  книжная), двигаются за ярлык и меняют размер за угол, хранятся в
  `miroCanvas.export` и никогда не становятся нодами — ни один элемент доски не
  двигается и не меняется. «Add page» ставит страницу вокруг выделения или в
  середине вида, «A page per frame» — вокруг каждой рамки. Открытие панели
  ничего не записывает; страницы сохраняются, когда их меняют. У презентации
  своя кнопка экспорта: её страницы — слайды, каждый своего размера (ничего не
  сохраняется). Каждая страница снимается так же, как нативный «Export as
  image»: доска на всё окно, по одному тайлу размером с окно, 2000 или 3000
  пикселей по ширине — и собирается в PDF (лист на страницу) или PowerPoint
  (картинка на слайд, с именем страницы). Маленькое окно поверх Obsidian
  показывает ход и позволяет остановить экспорт. Кнопки плагина, контуры
  страниц и выделение в картинки не попадают, вид возвращается на прежнее
  место. Нужен Obsidian на компьютере.

  Попутно: доска, записанная другим инструментом (конвертер пишет стрелку
...
- [x] `FUT-011` `P1 P` Ознакомительная доска, которую пользователь получает по
  ...
  стикеры, фигуры и блок-схемы, линии, рисование, слои и блокировка,
  комментарии, код и таблицы, экспорт, перенос из Miro, - которые показывают, а
  не рассказывают, из тех же записей, что пишут инструменты; её предлагает
  вопрос при первом запуске, а Настройки → «Начало работы» создаёт снова;
  существующая доска с тем же именем открывается, а не перезаписывается.
  Дополнено 2026-09-25: фрейм «Файлы и заметки» показывает собственные
  файловые карточки Canvas - заметку, мини-холст, картинку, PDF и документ
  Word; создание доски заодно создаёт рядом папку с этими образцами, по тому
  же нажатию и только недостающие файлы. Образцы хранятся в base64 в
...
- [ ] `FUT-016` `P1 P` Шрифты. База сделана 2026-09-24: в списке только
  ...
  Obsidian; шрифт, которого на машине нет (из старых досок или из Miro),
  показывается похожим того же рода, а не Times New Roman. Дальше - наборы
  шрифтов, которые пользователь сам скачивает и удаляет в настройках плагина:
  - набор Miro: открытые шрифты из списка Miro (Abril Fatface, Bangers, Caveat,
    EB Garamond, Fredoka One, Graduate, Gravitas One, IBM Plex Sans/Serif/Mono,
    Nixie One, Noto Sans, Open Sans, PT Sans, PT Sans Narrow, PT Serif,
    Permanent Marker, Rammetto One, Roboto, Roboto Condensed/Mono/Slab, Titan
    One и другие); закрытые шрифты Miro (Roobert, Spoof, Tiempos Text и
    подобные) заменяются близкими открытыми;
  - набор Word: открытые шрифты с метриками Calibri, Cambria, Arial, Times New
    Roman и Courier New (Carlito, Caladea, Liberation); сами шрифты Microsoft
    распространять нельзя;
  - набор Excalidraw: Excalifont, Virgil, Nunito, Lilita One, Comic Shanns и
    другие;
  - свои шрифты из файла (.ttf, .otf, .woff2), например ГОСТ;
  - свой список: оставить нужное, удалить лишнее, задать порядок в списке.
  ...
  --write-catalogue`, `src/settings.ts`, `src/settings-tab.ts`,
  `src/selection-toolbar.ts`, `src/appearance.ts`, `src/main.ts`): чтение
  несжатого ZIP и проверка манифеста без обращения к сети; скачивание по
  нажатию сверяет SHA-256 архива с встроенным каталогом и лишь затем
  распаковывает его в собственную папку плагина, с ходом загрузки в строке
  настройки. Один элемент `<style>` на каждое окно - основное и любой
  выносной - несёт правила `@font-face` установленных наборов и своих
  файлов шрифтов; правило для псевдонима (имя шрифта Word, собственное имя
  Excalidraw) сначала пробует шрифт самой системы, затем замену набора.
  Настройки → «Шрифты» показывают каждый набор с «Скачать»/«Удалить» и
  ходом загрузки, «Добавить файл шрифта» и собственный список шрифтов с
  переключателем и кнопками вверх/вниз. `MIRO_FONT_IDS` связывает
  собственные идентификаторы Miro и обычные имена шрифтов Windows, которые
  Miro иногда пишет прямо (`open_sans`, `times_new_roman`, `arial`,
  `Calibri` и другие), с шрифтом, который заменяет их набор, поэтому
  `fontStack` и `fontLabel` показывают и называют их, как только появляется
  подходящий набор или сам этот шрифт. Архивы наборов лежат в
  предварительном выпуске `fonts-0.0.3` на GitHub; его собирает и публикует
  `.github/workflows/fonts.yml` по метке `fonts-0.0.N`, и только если
  сборка в точности повторяет `src/font-pack-catalogue.ts`. «Последний
  выпуск» GitHub, который читает проверка обновлений, пропускает
  предварительные выпуски. BRAT читает первое число метки как версию и,
  добавляя или обновляя плагин, сначала пробует предварительные выпуски:
  прежний `fonts-2`, прочитанный как 2.0.0, обгонял плагин, и BRAT сообщал
  об отсутствии `manifest.json`, прежде чем перейти к настоящему выпуску;
  метка, прочитанная как 0.0.N, всегда ниже выпусков самого плагина. Если
  набора в выпуске нет, скачивание прямо говорит «этот набор ещё не
  выложен» вместо ошибки.
- [ ] `FUT-017` `P1 P` Проверить внутри плагина все виды ссылок и формул:
  вики-ссылки и Markdown-ссылки на заметки, заголовки и блоки, внешние адреса,
  встраивания (`![[...]]`) заметок, картинок, PDF и других холстов, ссылки из
  досок Miro, формулы LaTeX в строке и блоком - в показанной карточке, при
  редактировании, в стикерах, фигурах, таблицах, подписях линий, комментариях и
  в экспорте в PDF и PowerPoint; найденное исправить.
- [x] `FUT-018` `P1 P` Нижняя панель инструментов. Вернуть в быстрый доступ
  собственные «Карточку», «Заметку из хранилища» и «Файл из хранилища» Canvas,
  с перетаскиванием на доску, как в самом Canvas (сейчас они спрятаны в «Ещё»
  и не перетаскиваются); значок стикера сделать непохожим на значок карточки;
  в настройках плагина дать выбрать, какие инструменты стоят на панели и в
  каком порядке, остальные остаются в «Ещё». Сделано в плагине 2026-09-25
  (`src/quick-tools.ts`, `src/settings.ts`, `src/settings-tab.ts`): порядок
  панели хранится в настройке `toolbarItems` (старые переключатели
  `showLassoTool` и `showConnectorTool` переносятся в неё при первом же
  открытии); собственные кнопки Canvas переезжают на панель или в «Ещё» тем
  же элементом DOM, без копирования, так что перетаскивание на доску и клик
  продолжают работать; стикер - простой квадрат, фигура - квадрат с кругом,
  как в Miro, оба цветом значков (карточка Canvas сохраняет свой значок
  заметки);
  Настройки → «Нижняя панель» показывает каждый инструмент с переключателем
  «на панели», кнопками вверх/вниз и сбросом по умолчанию.
- [x] `FUT-019` `P1 P` Настройка панелей прямо на доске (расширено
  дважды 2026-09-25 по просьбе пользователя). Сделано в плагине 2026-09-25
  (`src/panel-layout.ts`, `src/panel-arrange.ts`, `src/settings.ts`,
  `src/m1-controls.ts`, `src/m1-session.ts`, `src/main.ts`): пункт
  «Настроить панели» в меню доски угловой панели и команда без сочетания
  клавиш по умолчанию включают режим с плашкой сверху («Перетаскивайте
  панели и инструменты», «Вернуть как было», «Готово»); нижнюю панель,
  строку значков угловой панели и миникарту - теперь свою отдельную
  панель, не строку значков - можно перетащить в новое место; место
  хранится как ближайший угол или край плюс отступ (`panelLayout`, запись
  на каждую панель; если записи нет, панель остаётся на своём месте по
  CSS), так что при изменении размера окна панель остаётся внутри вида и
  на своём месте, с прилипанием к краям и центральным линиям в пределах
  12 пикселей и переходом в вертикальный вид у бокового края. Инструменты
  внутри нижней панели можно переставить, убрать с неё в лоток того, чего
  на ней нет, или вернуть обратно - в ту же настройку `toolbarItems`,
  которую редактирует и список в настройках; второго источника истины нет.
  Escape, «Готово» или переход на другую доску выключают режим; пока он
  включён, доска игнорирует нажатия, как и раньше под каждой из этих
  панелей. Перемещение панели сохраняется лёгким путём без пересборки
  сессии; изменение нижней панели пересобирает её, как и раньше при
  правке из списка настроек, но режим остаётся включённым через такую
  пересборку на той же доске. Панель форматирования не входит в число
  перемещаемых - она, как и раньше, следует за выделением. Ориентацию
  панели теперь можно выбрать и напрямую, не только через угол: рядом с
  ручкой в режиме настройки у нижней панели и строки значков угловой
  панели появилась маленькая кнопка-разворот (иконка `rotate-cw`,
  «Повернуть панель»), которая сразу переключает панель между строкой и
  столбцом и пересчитывает её положение так, чтобы она осталась в пределах
  вида; выбор хранится как `orientation` в записи панели в `panelLayout` и
  перекрывает старое правило по краю только тогда, когда он действительно
  сделан. У вертикальной панели её собственные всплывающие окна - меню
  «Ещё», палитра фигур - и строки пера и линий теперь открываются в
  сторону, к середине доски, а не за край, где им уже не хватает места.
  Меню угловой панели вынесены из её потока и ставятся на место при
  каждом открытии (`placeDockMenu`, `src/m1-controls.ts`): над строкой или
  под ней, у столбца - сбоку, к середине доски, всегда в пределах вида,
  так что передвинутая панель больше не растёт за край, когда открывается
  меню. Кроме того, каждая панель заново встаёт на своё место на следующем
  кадре, когда меняется её собственный размер: место, найденное до того,
  как подгрузились стили, - при включении или обновлении плагина, -
  считалось по ещё не оформленной панели шириной во всю доску.
  Собственные кнопки Canvas - карточка, заметка, файл - и раньше создавали
  элемент перетаскиванием; теперь так делают и инструменты плагина (текст,
  стикер, фигура, комментарий, фрейм, блок кода, таблица, ссылка): нажатие
  на такую кнопку, на панели или в «+», и сдвиг указателя больше чем на
  4 экранных пикселя показывают силуэт элемента в его размере по
  умолчанию и с текущим масштабом доски, а отпускание над доской создаёт
  его там, по центру точки отпускания, тем же путём, что и щелчок с
  выбранным инструментом - одним шагом отмены, с выделением элемента
  после, точно как после щелчка. Отпускание над панелью, за пределами
  вида или нажатие Escape ничего не создают; обычный щелчок без сдвига
  просто выбирает инструмент, как и раньше.
- [ ] `FUT-020` `P2 P` Поиск по доске: текст карточек, стикеров, фигур, таблиц,
  подписей линий, комментариев и имена файлов; переход к найденному с
  подсветкой, следующее и предыдущее.
- [x] `FUT-021` `P1 P` Панель форматирования по образцу Miro. Сделано в плагине
  2026-09-25 (`src/selection-toolbar.ts`, `src/text-list.ts`, `src/text-link.ts`,
  `src/m1-session.ts`): группы карточки теперь идут фигура | шрифт, размер |
  начертание, выравнивание, список, ссылка | цвет текста, маркер, заливка,
  рамка | комментарий, блокировка, «Ещё». Линия или стрелка вместо этого
  показывает свои элементы (концы, вид, стиль, толщину, шрифт подписи) | цвет
  линии | комментарий, блокировка, «Ещё». В «Ещё» лежат пункты порядка, затем
  собственные добавки нативного меню - показать выделенное, редактировать,
  открыть ссылку, если выбрано превью ссылки - как те же нативные элементы,
  которые сессия туда переносит, и удаление последним, как опасное действие.
  Три новых элемента: переключатель маркированного списка (`- ` на каждой
  непустой строке текста выделенных карточек, блоки кода и таблицы не
  трогает), поле ссылки, которое превращает весь текст карточки - или его
  выделенную часть при редактировании - в markdown-ссылку (только http(s) и
  obsidian://, остальное тихо отклоняется; уже стоящая ссылка показана в поле
  и снимается пустым значением), и кнопка комментария, которая закрепляет
  новый комментарий прямо на выделенном - тот же результат, что даёт
  инструмент комментария, если щёлкнуть по этому элементу. Ничего из
  прежнего не потеряно, только перемещено.
- [ ] `FUT-022` `P1 P` Обновления. Правила Obsidian запрещают плагину
  обновлять самого себя, поэтому обновления приходят так: выпуски на GitHub
  (метка версии и файлы main.js, manifest.json, styles.css); до каталога
  сторонних плагинов - через плагин BRAT, который ставит и обновляет плагин из
  выпусков этого репозитория (описать в README); после попадания в каталог -
  обычной проверкой обновлений Obsidian. Сделано в плагине 2026-09-25
  (`src/update-check.ts`): раз в день при запуске, если это не выключено в
  настройках, и по кнопке «Проверить» плагин узнаёт у GitHub номер последнего
  выпуска; более новый выпуск появляется в строке состояния, а по щелчку -
  окно с его описанием, ссылкой и подсказкой, как обновиться. Ничего не
  скачивается. Осталось: первый выпуск и BRAT в README.
...
1. **Фундамент:** формальная версионированная схема `miroSource`/`miroCanvas` с
   эталонными файлами; система переводов с английским и русским и языком по
   Obsidian (`FUT-001`); лёгкий скилл для агентов — описание формата и
   проверка по схеме (`FUT-007`).
2. **Конвертер как самостоятельный продукт:** дедупликация вложений по SHA-256
   (`FUT-006`); экспорт в JSON, Canvas, Advanced Canvas и miro-canvas с простым
   GUI (`FUT-005`); сборки на Python под Windows, затем macOS и Linux
   (`FUT-013`).
3. **Доставка и мастер импорта:** плагин в отдельном репозитории, связанный с
   miro2obsidian схемой и эталонами (`FUT-002`); вопрос «Импортировать из Miro?»
   при первой настройке, инструкция с картинками, кнопка в настройках
   (`FUT-003`, `FUT-014`).
4. **Знакомство с плагином:** ознакомительная доска по желанию (`FUT-011`);
   нижняя панель инструментов (`FUT-018`), панель форматирования по образцу
   Miro (`FUT-021`) и наборы шрифтов (`FUT-016`) - до снимков экрана, чтобы
   они не устарели; затем визуальное описание функций и порядка настройки на
   обоих языках, со снимками с этой доски (`FUT-012`).
5. **Проверка на людях:** полный путь пользователя на чистой Windows и
   исправления (`FUT-004`); все виды ссылок и формул (`FUT-017`); затем
   другие ОС, телефоны и планшеты (`FUT-010`).
6. **Экосистема:** MCP-сервер рядом со скиллом (`FUT-007`), импорт из других
   плагинов (`FUT-009`), перемещаемые панели (`FUT-019`), поиск по доске
