...
- [ ] `FUT-021` `P1 P` Панель форматирования по образцу Miro: те же группы и
  порядок - фигура | шрифт и размер | начертание, выравнивание, список,
  ссылка | цвет текста, маркер, заливка | комментарий, блокировка, «⋮ Ещё»
  (порядок, удаление, копирование и остальное). Сейчас рамка, порядок,
  удаление, «показать выделенное» и «редактировать» стоят на панели отдельными
  кнопками, а списка, ссылки и комментария на ней нет.
