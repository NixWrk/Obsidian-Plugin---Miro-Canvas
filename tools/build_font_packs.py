...
def build(pack: Pack, fetcher: Fetcher, metadata: dict | None, out: Path) -> Path:
    faces: list[Face] = []
...
def main() -> int:
    ...
    catalogue = []
    for pack in chosen:
        archive = build(pack, fetcher, metadata, args.out)
        catalogue.append({"id": pack.id, "file": archive.name, "bytes": archive.stat().st_size, "sha256": hashlib.sha256(archive.read_bytes()).hexdigest()})
    (args.out / "catalogue.json").write_text(json.dumps(catalogue, indent=1), encoding="utf-8")
    return 0
